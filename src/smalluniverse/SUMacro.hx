...
class SUMacro {
	public static macro function jsx(expr:ExprOf<String>):Expr {
		...
		return children(tink.hxx.Parser.parseRoot(expr, {
	...
	public static macro function getExtraProps(props:Expr):Expr {
		return switch (Context.typeof(props)) {
			...
			default:
				Context.fatalError('getExtraProps() expects an anonymous structure as type of argument', Context.currentPos());
	...
	static public function toFieldExpr(sl:Array<String>, pos:Position = null):Expr {
		...
		return Lambda.fold(sl, function(s, e) {
			return e == null ? (macro @:pos(pos) $i{s}) : (macro @:pos(pos) $e.$s);
		}, null);
	}
	...
	static function children(c:tink.hxx.Children, type:ComplexType) {
		var exprs = switch (c) {
			...
			case _ if (type == REACT_FRAGMENT_CT):
				[
					for (c in tink.hxx.Generator.normalize(c.value))
						macro @:pos(c.pos) (${child(c)} : $type)
				];
	...
	static function typeChecker(type:Expr, nodePos:Position, isHtml:Bool):StringAt->Expr->Expr {
		function propsFor(placeholder:Expr):StringAt->Expr->Expr {
			...
			return function(name:StringAt, value:Expr) {
					...
					if (reg.match(e.toString())) {
						switch (Context.typeof(type)) {
							case TType(_.get().type => TAnonymous(_.get() => {
								...
								if (validators[0].params.length > 0) {
									for (v in validators[0].params) {
										...
										if (k == null) {
											Context.error('Unexpected argument. Expected no argument or ' + 'an identifier to a registered props validator',
												v.pos);
										}

										var validator = PropsValidator.get(k);
										if (validator == null) {
											Context.error('Error: cannot find props validator "$k"', v.pos);
										} else {
							...
							default:
								...
								if (StringTools.startsWith(field, "data-"))
									return value;
								#end
				...
				if (isTMono) {
					...
					Context.warning('Type checking failed: unable to infer ' + 'needed props for ${ExprTools.toString(type)}', type.pos);
					#end
		...
		return isHtml ? function(name:StringAt, value:Expr) {
	...
	static function extractChildrenType(type:Expr):ComplexType {
		try {
			switch (Context.typeof(type)) {
				...
				case TFun([{t: _ => Context.follow(_) => TAnonymous(_.get().fields => fields)}],
					_), TFun([{t: TType(_.get() => _.type => TAnonymous(_.get().fields => fields), _)}], _):
					var childType = extractChildrenTypeFromFields(fields);
					if (isReactFragment(childType))
				...
				default:
					Context.warning('Cannot get children type for ${ExprTools.toString(type)}', type.pos);
			}
		} catch (e:Dynamic) {}
	...
	static function extractChildrenTypeFromFields(fields:Array<{name:String, type:Type}>):Null<ComplexType> {
		for (f in fields)
			if (f.name == 'children')
		...
		return switch (c.value) {
			...
			case CNode(n):
				...
				for (attr in n.attributes) {
					switch (attr) {
						...
						case Regular(name, value):
							neededAttrs.remove(name.value);
							var expr = value.getString().map(function(s) return haxe.macro.MacroStringTools.formatString(s, value.pos)).orUse(value);
				...
				for (attr in neededAttrs)
					Context.warning('Missing prop `$attr` for component `${n.name.value}`', c.pos);
	...
	static function genLiteral(type:Expr, props:Expr, ref:Expr, key:Expr, pos:Position) {
		...
		if (key != null)
			fields.push({field: 'key', expr: key});
		if (ref != null)
			fields.push({field: 'ref', expr: ref});
	...
	static function canUseLiteral(typeInfo:ComponentInfo, ref:Expr) {
		...
		if (typeInfo != null && typeInfo.isExtern)
			return false;
	...
	static function makeProps(spread:Array<Expr>, attrs:Array<ObjectField>, pos:Position) {
		...
		return spread.length > 0 ? makeSpread(spread, attrs, pos) : attrs.length == 0 ? macro {} : {pos: pos, expr: EObjectDecl(attrs)}
	}
	...
	static function makeSpread(spread:Array<Expr>, attrs:Array<ObjectField>, pos:Position) {
		...
		if (attrs.length > 0)
			args.push({pos: pos, expr: EObjectDecl(attrs)});
		...
		return switch (last.expr) {
			case ECheckType({expr: EObjectDecl(fields)}, ct):
				...
				var newProps = props.concat(fields.map(function(f) {
					var fname = f.field;
					var fct = TypeTools.toComplexType(Context.typeof(macro @:pos(f.expr.pos) (null : $ct).$fname));
				...
				switch (field.kind) {
					case FieldType.FVar(ct, _.expr => EObjectDecl(props)):
						if (ct == null) {
							...
							var tprops = TypeTools.toComplexType(Context.follow(ComplexTypeTools.toType(types.tprops)));
							...
							ct = TypeTools.toComplexType(Context.follow(ComplexTypeTools.toType(ct)));
						}
...
// #if client
// typedef SUMacro = react.ReactMacro;
// #else
// #end
// /**
